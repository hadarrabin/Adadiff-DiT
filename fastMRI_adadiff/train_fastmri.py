...
class UnifiedMRIDataset(Dataset):
    def __init__(self, data_dir, dataset_type: int, virtual_coils: int = 5, acceleration: int = 4, input_size: int = 256):
    ...
    def _process_fastmri(self, filepath):
        ...
        resized_kspace_imag = F.interpolate(compressed_kspace_imag, size=(self.input_size, self.input_size), mode='bilinear', align_corners=False)
...
# =============================================================================
# Default fallbacks if no checkpoint exists
start_epoch = 15
start_step = 30  # Default to Step 30 as requested

CHECKPOINT_FILE = "/ugproj/sipl-prj10848/Idan/fastmri_checkpoints/adadiff_fastmri_epoch_15_step_30.pt"
...
if os.path.exists(CHECKPOINT_FILE):
    ...
    # Auto-extract parameters from metadata
    ...
    print(f"Beginning training from scratch (Fallback: Epoch {start_epoch}, Step 0)...")
    start_step = 0
...
CHECKPOINT_DIR = "/ugproj/sipl-prj10848/Idan/fastmri_checkpoints"
...
# Optimized for Local HDF5 files using 4 CPU workers
...
for epoch in range(start_epoch, NUM_EPOCHS + 1):
    ...
    for batch_idx, clean_x0 in enumerate(train_loader):
        # Determine the current completed optimization step for this iteration
        # (Since 1 step = ACCUMULATION_STEPS batches)
        current_step = (batch_idx + 1) // ACCUMULATION_STEPS
        
        # --- THE STEP-SKIPPING ENGINE ---
        # Skip steps that were already calculated in our loaded checkpoint
        ...
        if (batch_idx + 1) % ACCUMULATION_STEPS == 0:
            ...
            if current_step % SAVE_EVERY_N_STEPS == 0:
                step_path = os.path.join(CHECKPOINT_DIR, f"adadiff_fastmri_epoch_{epoch}_step_{current_step}.pt")
                torch.save({
                    'epoch': epoch,
                    'step': current_step,
    ...
    if epoch % SAVE_EVERY_N_EPOCHS == 0:
        epoch_path = os.path.join(CHECKPOINT_DIR, f"adadiff_fastmri_epoch_{epoch}_final.pt")
        torch.save({
            'epoch': epoch,
            'step': num_steps,
            'generator_state_dict': dit_backbone.state_dict(),
