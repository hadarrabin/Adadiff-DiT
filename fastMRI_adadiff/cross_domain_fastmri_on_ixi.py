...
def load_and_preprocess_volume(file_path, target_channels=1, target_size=320):
    """
    ...
    # Spatial Adaptation: Center crop or Pad IXI slices to match target_size (e.g., 320x320)
    B, C, H, W = tensor_data.shape
...
if __name__ == "__main__":
    ...
    print(f"🚀 Running CROSS-DOMAIN Evaluation (fastMRI Model -> IXI Samples) on: {str(device).upper()}")
    
    # 1. Load fastMRI winning hyperparameters (Paper Cross-Domain Protocol)
    ...
    # Limit to 5 volumes per contrast for rapid execution (~1 hour total run)
    MAX_VOLUMES_PER_CONTRAST = 5
    for c in test_set:
        test_set[c] = test_set[c][:MAX_VOLUMES_PER_CONTRAST]

    print("\n" + "="*50)
    print("📊 CROSS-DOMAIN TEST SUBSET INVENTORY:")
        ...
        for contrast, file_paths in test_set.items():
            ...
            for vol_idx, fpath in enumerate(file_paths):
                ...
                for slice_idx in range(volume_data.shape[0]):
                    ...
                    # Energy Guard: Only consider anatomical slices for report visualization
            ...
            if best_visual_data is not None:
                ...
                fig.suptitle(
                    f"Cross-Domain (fastMRI -> IXI) Best Reconstruction: {contrast} (R={R}x)\n"
                ...
                plt.tight_layout()
                plt.savefig(f"cross_domain_golden_sample_{contrast}_R{R}.png", dpi=300, bbox_inches='tight')
                plt.close()

    with open("fastmri_to_ixi_cross_domain_results.json", "w") as f:
        json.dump(final_results, f, indent=4)

    print("\n" + "="*80)
    print("🏆 FINAL CROSS-DOMAIN (fastMRI -> IXI) RESULTS SUMMARY")
    print("="*80)
    ...
    for r_key, r_data in final_results.items():
        ...
        print(r"\hline")
    print(r"\end{tabular}")
    print(r"\caption{Cross-domain (fastMRI model evaluated zero-shot on IXI test slices) reconstruction metrics.}")
    print(r"\label{tab:cross_domain_results}")
    print(r"\end{table}")
    print("="*80)
    print("\n✅ Cross-domain evaluation complete! Results saved to 'fastmri_to_ixi_cross_domain_results.json'.")
