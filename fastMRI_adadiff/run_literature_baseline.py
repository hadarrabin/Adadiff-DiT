...
# =============================================================================
NUM_VOLUMES_PER_CONTRAST = 3  # Change to 5 if you want a larger subset
RANDOM_SEED = 42              # Guarantees identical patient selection every run

BASELINE_CONFIG = {
    "lr": 0.0001,             # Standard conservative literature step size
    "J": 1000,                # Full AdaDiff paper adaptation steps
    "eta": 1.0,               # Exact data consistency
    "scope": "adaln_only"     # Signature AdaDiff modulation scope
}
...
def load_trained_generator(checkpoint_path, device="cuda"):
    ...
    print(f"📦 Auto-detected fastMRI Checkpoint Architecture:")
...
# =============================================================================
# 2. MAIN EVALUATION SCRIPT FOR LITERATURE SUBSET
# =============================================================================
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run AdaDiff Literature Baseline on Subset")
    parser.add_argument("--modality", type=str, default="ALL", choices=["ALL", "T1", "T2", "FLAIR"],
                        help="Select a single modality to evaluate, or 'ALL' to run everything sequentially.")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🚀 Running Literature Baseline (J={BASELINE_CONFIG['J']}) on: {str(device).upper()}")
    if args.modality != "ALL":
        print(f"🎯 MODALITY ISOLATION ENABLED: Only evaluating [{args.modality}]")
    ...
    target_contrasts = ["T1", "T2", "FLAIR"] if args.modality == "ALL" else [args.modality]
    
    test_set = {c: [] for c in target_contrasts}
    ...
    # 🛡️ DETERMINISTIC SUBSET SELECTION
    print("\n" + "="*50)
    print(f"🎲 DETERMINISTIC SUBSET SELECTION (Seed={RANDOM_SEED}, N={NUM_VOLUMES_PER_CONTRAST} vols/contrast):")
    total_volumes = 0
    for c in target_contrasts:
        sorted_paths = sorted(test_set[c])
        if len(sorted_paths) > NUM_VOLUMES_PER_CONTRAST:
            random.seed(RANDOM_SEED)
            test_set[c] = random.sample(sorted_paths, NUM_VOLUMES_PER_CONTRAST)
        print(f"   -> Modality [{c}]: Selected {len(test_set[c])} patient volumes")
        total_volumes += len(test_set[c])
    ...
    for R in acceleration_rates:
        print(f"\n=========================================================")
        print(f" EVALUATING LITERATURE BASELINE AT R = {R}x")
        print(f" Config: lr={BASELINE_CONFIG['lr']}, J={BASELINE_CONFIG['J']}, eta={BASELINE_CONFIG['eta']}, scope={BASELINE_CONFIG['scope']}")
        print(f"=========================================================")
        ...
        for contrast, file_paths in test_set.items():
            ...
            for vol_idx, fpath in enumerate(file_paths):
                volume_data = load_and_preprocess_volume(fpath, target_channels=base_model.in_channels, target_size=base_model.input_size)
                ...
                for slice_idx in range(volume_data.shape[0]):
                    ...
                    recon, exec_time = run_prior_adaptation(
                        ...
                        J=BASELINE_CONFIG["J"], lr=BASELINE_CONFIG["lr"], eta=BASELINE_CONFIG["eta"], 
                        param_scope=BASELINE_CONFIG["scope"], device=device
                    )
            ...
            if best_visual_data is not None:
                ...
                fig.suptitle(f"Literature Baseline (J={BASELINE_CONFIG['J']}): {contrast} (R={R}x)\nPSNR: {best_visual_data['psnr']:.2f} dB | SSIM: {best_visual_data['ssim']*100:.2f}%", fontsize=13, fontweight='bold')
                
                t_img, r_img = best_visual_data["true"], best_visual_data["recon"]
                e_img = np.abs(t_img - r_img)

                axes[0].imshow(t_img, cmap='gray', vmin=0, vmax=1); axes[0].set_title("Ground Truth"); axes[0].axis("off")
                axes[1].imshow(best_visual_data["aliased"], cmap='gray', vmin=0, vmax=1); axes[1].set_title("Zero-Filled Input"); axes[1].axis("off")
                axes[2].imshow(r_img, cmap='gray', vmin=0, vmax=1); axes[2].set_title("DiT Reconstructed"); axes[2].axis("off")
                im_err = axes[3].imshow(e_img, cmap='hot', vmin=0, vmax=0.15); axes[3].set_title("Absolute Error Map"); axes[3].axis("off")
                fig.colorbar(im_err, ax=axes[3], fraction=0.046, pad=0.04)
                
                plt.tight_layout(); plt.savefig(f"baseline_golden_sample_{contrast}_R{R}.png", dpi=300); plt.close()

    # Save results dictionary safely
    output_fname = "literature_baseline_results.json" if args.modality == "ALL" else f"literature_baseline_{args.modality}.json"
    with open(output_fname, "w") as f:
        json.dump(final_results, f, indent=4)

    print("\n" + "="*80)
    print("🏆 FINAL LITERATURE BASELINE SUMMARY (ADADIFF TABLE 2 PARITY)")
    print("="*80)
    print("\n--- MARKDOWN FORMAT ---")
    ...
    for r_key, r_data in final_results.items():
        ...
        for c_key, m in r_data.items():
            print(f"| **{r_num}x** | **{c_key}** | {m['PSNR_mean']:.2f} ± {m['PSNR_std']:.2f} | {m['SSIM_mean']:.2f} ± {m['SSIM_std']:.2f} | {m['LPIPS_mean']:.4f} ± {m['LPIPS_std']:.4f} | {m['Time_mean']:.2f} ± {m['Time_std']:.2f} |")
            
    print("\n✅ Evaluation complete! Results saved to", output_fname)