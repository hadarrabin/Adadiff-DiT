...
# =============================================================================
# 2. DATASET PROCESSING (SPATIAL COIL DOMAIN + NORMALIZED TO [-1, 1])
# =============================================================================
class UnifiedMRIDataset(Dataset):
    def __init__(self, data_dir, dataset_type: int, virtual_coils: int = 5, input_size: int = 256):
    ...
    def _process_fastmri(self, filepath):
        ...
        compressed_kspace = torch.matmul(compression_matrix, flattened_kspace).view(self.virtual_coils, h, w)

        # Bilinear Interpolation to input_size
        ...
        # Transform k-space to spatial image space
        spatial_coils = torch.fft.ifft2(torch.fft.ifftshift(resized_kspace.unsqueeze(0), dim=(-2, -1)), dim=(-2, -1), norm="ortho").squeeze(0)

        # Normalize physical spatial pixel intensities cleanly to [-1.0, 1.0]
        max_val = torch.max(torch.abs(spatial_coils))
        if max_val > 1e-8:
            spatial_coils = spatial_coils / max_val

        return torch.cat([spatial_coils.real, spatial_coils.imag], dim=0)
    ...
    def _process_ixi(self, filepath):
        ...
        return (tensor_data - min_val) / (max_val - min_val + 1e-8) * 2.0 - 1.0  
...
class DiffusionTransformer(nn.Module):
    def __init__(self, in_channels: int, input_size: int, patch_size: int = 4, hidden_dim: int = 384, depth: int = 12, num_heads: int = 6):
        super().__init__()
...
# =============================================================================
# 6. FOURIER DUAL-DOMAIN CONVERTER FOR HYBRID LOSS
# =============================================================================
def spatial_to_kspace_tensor(spatial_tensor: torch.Tensor) -> torch.Tensor:
    """Converts 10-channel spatial coils into orthogonal k-space for hybrid frequency loss."""
    num_coils = spatial_tensor.shape[1] // 2
    spatial_complex = torch.complex(spatial_tensor[:, :num_coils], spatial_tensor[:, num_coils:])
    kspace_complex = torch.fft.fftshift(torch.fft.fft2(spatial_complex, dim=(-2, -1), norm="ortho"), dim=(-2, -1))
    return torch.cat([kspace_complex.real, kspace_complex.imag], dim=1)

# =============================================================================
# 7. PIPELINE INITIALIZATION & ENVIRONMENT CONFIGURATION
# =============================================================================
if __name__ == "__main__":
    torch.cuda.empty_cache()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Running accelerated fastMRI pipeline on target device: {str(device).upper()}")
    ...
    DRIVE_FASTMRI_TRAIN = "/ugproj/sipl-prj10848/Idan/fastMRI_adadiff/train"
    CHECKPOINT_DIR = "/ugproj/sipl-prj10848/Idan/fastmri_checkpoints_sinusoidal"
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    CHECKPOINT_FILE = os.path.join(CHECKPOINT_DIR, "adadiff_fastmri_sinusoidal_latest.pt")

    in_channels_G = 2 * VIRTUAL_COILS if DATASET_TYPE == 0 else 1

    print("\nInitializing model parameters for high-speed fastMRI sinusoidal training...")
    dit_backbone = DiffusionTransformer(in_channels=in_channels_G, input_size=INPUT_SIZE).to(device)
    discriminator = AdaDiffDiscriminator(dataset_type=DATASET_TYPE, virtual_coils=VIRTUAL_COILS).to(device)
    scheduler = AdaDiffScheduler(step_size_k=125, total_steps_T=1000)

    # UPDATED: TTUR Optimization (Discriminator LR is half of Generator LR)
    ...
    # UPDATED: Cosine Schedulers for smooth convergence
    ...
    if os.path.exists(CHECKPOINT_FILE):
        ...
        # Safely load scheduler states if resuming
        ...
        print(f"🚀 Weights successfully restored! Resuming from Epoch {start_epoch}, Step {start_step}.\n")
    else:
        print("Starting fresh sinusoidal fastMRI training sprint from scratch...")

    train_dataset = UnifiedMRIDataset(data_dir=DRIVE_FASTMRI_TRAIN, dataset_type=0, input_size=INPUT_SIZE)
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)

    print(f"Loaded training setup containing {len(train_dataset)} scans.")
    print(f"Effective Batch Size: {BATCH_SIZE * ACCUMULATION_STEPS} | AMP Tensor Cores Active: True")

    # =============================================================================
    # 8. AMP-ACCELERATED ADV_TRAINING LOOP (SPATIAL DOMAIN + HYBRID LOSS)
    # =============================================================================
    dit_backbone.train()
    discriminator.train()
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        for batch_idx, clean_x0 in enumerate(train_loader):
            ...
            with torch.amp.autocast('cuda'):
                ...
                with torch.no_grad():
                    x_fake_target = dit_backbone(x_parent, t_parent_tensor)
            ...
            # ---------------------------------------------------------------------
            # GENERATOR STREAM WITH DUAL-DOMAIN HYBRID CONTENT LOSS
            # ---------------------------------------------------------------------
            with torch.amp.autocast('cuda'):
                x_fake_train = dit_backbone(x_parent, t_parent_tensor)
                ...
                kspace_fake = spatial_to_kspace_tensor(x_fake_train)
                kspace_real = spatial_to_kspace_tensor(x_real_target)
                loss_G_kspace = F.l1_loss(kspace_fake, kspace_real) / ACCUMULATION_STEPS

                # UPDATED: Total Hybrid Objective (K-Space Multiplier increased to 20.0 for sharper edges)
                loss_G_step = loss_G_adv + (10.0 * loss_G_spatial) + (20.0 * loss_G_kspace)
            ...
            if (batch_idx + 1) % ACCUMULATION_STEPS == 0:
                ...
                if current_step % SAVE_EVERY_N_STEPS == 0:
                    step_path = os.path.join(CHECKPOINT_DIR, "adadiff_fastmri_sinusoidal_latest.pt")
                    torch.save({
                ...
                if current_step % PRINT_EVERY_N_BATCHES == 0:
                    # Print current learning rate to monitor Cosine Decay
        ...
        print(f"\n=========================================================")
        print(f"🏁 FASTMRI EPOCH {epoch}/{NUM_EPOCHS} COMPLETED ({elapsed:.1f}s)")
        print(f"=========================================================")
        ...
        # UPDATED: Step the learning rate down smoothly
        scheduler_lr_G.step()
        scheduler_lr_D.step()

        if epoch % SAVE_EVERY_N_EPOCHS == 0:
            epoch_path = os.path.join(CHECKPOINT_DIR, f"adadiff_fastmri_sinusoidal_epoch_{epoch}.pt")
            save_dict = {
            ...
            print(f"💾 Saved permanent end-of-epoch fastMRI checkpoint to: {epoch_path}\n")
