...
if __name__ == "__main__":
    ...
    print(f"🚀 Running CROSS-DOMAIN V2 (REPEATED CHANNELS) on: {str(device).upper()}")
    ...
    print("\n" + "="*50)
    print("📊 CROSS-DOMAIN V2 TEST SUBSET INVENTORY:")
    ...
    for R in acceleration_rates:
        ...
        for contrast, file_paths in test_set.items():
            ...
            # Save distinct images with '_repeated_' suffix
            if best_visual_data is not None:
                fig, axes = plt.subplots(1, 4, figsize=(16, 4.2))
                fig.suptitle(
                    f"Cross-Domain V2 (Repeated Channels) Best Recon: {contrast} (R={R}x)\n"
                ...
                plt.tight_layout()
                plt.savefig(f"cross_domain_repeated_golden_sample_{contrast}_R{R}.png", dpi=300, bbox_inches='tight')
                plt.close()

    # Save distinct JSON file for repeated channel experiment
    with open("fastmri_to_ixi_cross_domain_results_repeated.json", "w") as f:
        json.dump(final_results, f, indent=4)

    print("\n" + "="*80)
    print("🏆 FINAL CROSS-DOMAIN V2 (REPEATED CHANNELS) RESULTS SUMMARY")
    print("="*80)
    ...
    for r_key, r_data in final_results.items():
        ...
        print(r"\hline")
    print(r"\end{tabular}")
    print(r"\caption{Cross-domain V2 (repeated channels) reconstruction metrics.}")
    print(r"\label{tab:cross_domain_v2_results}")
    print(r"\end{table}")
    print("="*80)
    print("\n✅ Execution complete! Results saved to 'fastmri_to_ixi_cross_domain_results_repeated.json' and 'cross_domain_repeated_golden_sample_*.png'.")