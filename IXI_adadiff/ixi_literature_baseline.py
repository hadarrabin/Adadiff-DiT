...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run AdaDiff Literature Baseline on IXI Subset")
    parser.add_argument("--modality", type=str, default="ALL", choices=["ALL", "PD", "T2", "T1"],
                        help="Select a single modality to evaluate, or 'ALL' to run everything sequentially.")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🚀 Running Literature Baseline (J={BASELINE_CONFIG['J']}) on: {str(device).upper()}", flush=True)
    if args.modality != "ALL":
        print(f"🎯 MODALITY ISOLATION ENABLED: Only evaluating [{args.modality}]", flush=True)
    
    loss_fn_alex = lpips.LPIPS(net='alex').to(device).eval()
    
    # Updated to your actual IXI checkpoint path
    ...
    # Updated to your IXI test directory
    test_base_dir = "/ugproj/sipl-prj10848/Idan/IXI_adadiff/test"
    target_contrasts = ["PD", "T2", "T1"] if args.modality == "ALL" else [args.modality]
    ...
    print("\n" + "="*50, flush=True)
    print(f"🎲 DETERMINISTIC SUBSET SELECTION (Seed={RANDOM_SEED}, N={NUM_VOLUMES_PER_CONTRAST} vols/contrast):", flush=True)
    total_volumes = 0
    for c in target_contrasts:
        ...
        print(f"   -> Modality [{c}]: Selected {len(test_set[c])} patient volumes", flush=True)
    ...
    for R in acceleration_rates:
        print(f"\n=========================================================", flush=True)
        print(f" EVALUATING LITERATURE BASELINE AT R = {R}x", flush=True)
        print(f" Config: lr={BASELINE_CONFIG['lr']}, J={BASELINE_CONFIG['J']}, eta={BASELINE_CONFIG['eta']}, scope={BASELINE_CONFIG['scope']}", flush=True)
        print(f"=========================================================", flush=True)
        ...
        for contrast, file_paths in test_set.items():
            ...
            for vol_idx, fpath in enumerate(file_paths):
                ...
                for slice_idx in range(volume_data.shape[0]):
                    ...
                    # 🚀 Skip empty edge slices to save 1000 pointless adaptation steps
                    ...
                    if valid_slices_processed % 5 == 0 or slice_idx == volume_data.shape[0] - 1:
            ...
            print(f"  -> [{contrast} FINAL] PSNR: {mean_psnr:.2f} ± {std_psnr:.2f} dB | Time: {mean_time:.2f}s per slice", flush=True)
            ...
            if best_visual_data is not None:
                ...
                plt.tight_layout(); plt.savefig(f"ixi_baseline_golden_sample_{contrast}_R{R}.png", dpi=300, bbox_inches='tight'); plt.close()

    # Save results dictionary safely
    output_fname = "ixi_literature_baseline_results.json" if args.modality == "ALL" else f"ixi_literature_baseline_{args.modality}.json"
    with open(output_fname, "w") as f:
        json.dump(final_results, f, indent=4)

    print("\n" + "="*80, flush=True)
    print("🏆 FINAL IXI LITERATURE BASELINE SUMMARY", flush=True)
    print("="*80, flush=True)
    ...
    print(r"\caption{IXI baseline metrics (J=1000).}", flush=True)
    print(r"\label{tab:ixi_baseline_results}", flush=True)
    print(r"\end{table}", flush=True)
    print("\n✅ Evaluation complete! Results saved to", output_fname, flush=True)
