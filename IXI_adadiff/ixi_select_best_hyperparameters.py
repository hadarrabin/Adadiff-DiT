...
def process_grid_results(output_path="best_hyperparameters_ixi.json"):
    # Automatically locate the correct IXI grid summary file
    possible_files = [
        "ixi_adadiff_grid_summary.json", 
        "dit_adadiff_grid_summary.json", 
        "grid_summary_ixi.json",
        "adadiff_ixi_grid_summary.json"
    ]
    json_path = None
    ...
    if not json_path:
        print("❌ Error: Could not find any IXI grid summary JSON file in the current directory.")
        print("Please ensure your grid search results JSON is in this folder, or rename it to 'ixi_adadiff_grid_summary.json'.")
        return

    print(f"📂 Loading IXI grid search data from: '{json_path}'")
    ...
    for r_val, configs in r_groups.items():
        ...
        print(f"\n========================================")
        print(f" Evaluating IXI R={r_val} Configurations ({len(configs)} total)")
        print(f"========================================")
        ...
        # Case-insensitive key extraction with fallbacks
    ...
    print(f"\n✅ Best IXI hyperparameters successfully exported to '{output_path}'")
